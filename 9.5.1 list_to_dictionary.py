"""
Ejercicio 9.5.1. Escribir una función que reciba una lista de tuplas, y que devuelva un diccio-
nario en donde las claves sean los primeros elementos de las tuplas, y los valores una lista con
los segundos.
Por ejemplo:
>>> l = [ ('Hola', 'don Pepito'), ('Hola', 'don Jose'),
('Buenos', 'días') ]
>>> print(tuplas_a_diccionario(l))
{ 'Hola': ['don Pepito', 'don Jose'], 'Buenos': ['días'] }
"""

def list_of_tuples_to_dictionary(list_of_tuples: list):
    dictionary = {}
    for tuples in list_of_tuples:
        if tuples[0] not in dictionary:
            dictionary[tuples[0]] = []
        dictionary[tuples[0]].append(tuples[1])

    return dictionary

print(list_of_tuples_to_dictionary([('Hola', 'don Pepito'), ('Hola', 'don Jose'),('Buenos', 'días')]))
